...
import MaterialForm from './productform/MaterialForm';
import { formatCurrency } from '../lib/utils';
import { X, ToggleRight, ToggleLeft, FileText, Calculator, Utensils, Layers } from 'lucide-react';
import * as categoryService from '../services/categoryService';
import ProductImagesUpload from './productform/ProductImagesUpload';
import { useAuth } from '../context/AuthContext';
...
const ProductForm: React.FC<ProductFormProps> = ({
  ...
  // --- Deklarasi SEMUA STATE di bagian paling atas ---
  // State utama produk, diinisialisasi dengan nilai default
  const [product, setProduct] = useState<Product>({
    id: '',
    name: '',
    description: '',
  ...
  // State untuk form 'Materials'
  const [paperType, setPaperType] = useState<string | null>(null);
  const [paperGrammar, setPaperGrammar] = useState<string | null>(null);
  const [materialCostPerCm2, setMaterialCostPerCm2] = useState(0); 
  const [isPaperEnabled, setIsPaperEnabled] = useState(false);
  
  const [printType, setPrintType] = useState<'Black & White' | 'Full Color' | null>('Full Color');
  const [printCostPerCm2, setPrintCostPerCm2] = useState(0); 
  const [isPrintingEnabled, setIsPrintingEnabled] = useState(false);
  
  const [finishingType, setFinishingType] = useState<'Tanpa Finishing' | 'Doff' | 'Glossy' | 'Lainnya' | null>('Tanpa Finishing');
  const [customFinishing, setCustomFinishing] = useState<string | null>(null);
  const [finishingCostPerCm2, setFinishingCostPerCm2] = useState(0); 
  const [isFinishingEnabled, setIsFinishingEnabled] = useState(false);
  
  // State untuk form 'Pricing'
  const [markup, setMarkup] = useState(100);
  
  // State untuk form 'Recipe'
  const [recipeIngredients, setRecipeIngredients] = useState<RecipeIngredient[]>([]);

  // --- Semua fungsi dan useEffect sekarang dapat mengakses state ini dengan aman ---

  // Fetch categories saat komponen pertama kali di-mount
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const categories = await categoryService.getCategories();
  ...
  // Menginisialisasi semua state formulir saat 'initialProduct' berubah.
  useEffect(() => {
    const controller = new AbortController();
    let mounted = true;

    if (initialProduct) {
      console.log('--- useEffect [initialProduct] Triggered ---');
      ...
      // Sinkronisasi state utama
      setProduct(productToSet);
      
      // Sinkronisasi semua state individual
      setRecipeIngredients(productToSet.ingredients || []);
      setPaperType(productToSet.paper_type || null);
      setPaperGrammar(productToSet.paper_grammar || null);
      setIsPaperEnabled(productToSet.is_paper_enabled ?? false);
      setPrintType((productToSet.print_type as 'Black & White' | 'Full Color' | null) || 'Full Color');
      setIsPrintingEnabled(productToSet.is_printing_enabled ?? false);
      setFinishingType((productToSet.finishing_type as 'Tanpa Finishing' | 'Doff' | 'Glossy' | 'Lainnya' | null) || 'Tanpa Finishing');
      setCustomFinishing(productToSet.custom_finishing || null);
      setIsFinishingEnabled(productToSet.is_finishing_enabled ?? false);

      // Hitung dan atur markup
      if (productToSet.cost_price && parseFloat(productToSet.cost_price) > 0) {
        const modalPrice = parseFloat(productToSet.cost_price);
        const sellingPrice = parseFloat(productToSet.price);
        setMarkup(Math.round(((sellingPrice - modalPrice) / modalPrice) * 100));
      ...
      onChange?.(productToSet);
    } else {
      // Jika tidak ada initialProduct, kembalikan ke nilai default
      setProduct({
        id: '',
        name: '',
        description: '',
    ...
    return () => {
      mounted = false;
      controller.abort();
      console.log('--- useEffect cleanup [initialProduct] ---');
    };
  }, [initialProduct, user?.branch_id]);

  // Fungsi untuk memperbarui biaya produk berdasarkan state
  const updateProductCosts = () => {
    if (!product) return;

    let totalCost = 0;

    // Hitung biaya bahan dari resep
    if (recipeIngredients.length > 0) {
      totalCost += recipeIngredients.reduce((sum, ingredient) => {
        if (!ingredient?.price_per_unit) {
          return sum;
    ...
    // Tambahkan biaya material jika diaktifkan
    if (isPaperEnabled && materialCostPerCm2 > 0) {
      totalCost += materialCostPerCm2;
    }
  ...
  // Efek untuk memanggil updateProductCosts saat ada state terkait yang berubah
  useEffect(() => {
    updateProductCosts();
  }, [
    materialCostPerCm2,
  ...
  const handleMarkupChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const newMarkup = parseInt(e.target.value, 10);
    setMarkup(isNaN(newMarkup) ? 0 : newMarkup);
  };
